#!/usr/bin/env python3
"""
Analyze n=7 trees to identify:
1. Which trees descend from n=6 parents
2. Which trees are special combinations
3. The partition structure of each tree
4. The complete compositional taxonomy

Expected: 48 trees at n=7 (A000081 sequence: 1, 1, 2, 4, 9, 20, 48, ...)
"""
...
# Generate trees for n=2 through n=7
...
print("=" * 80)
print("N=7 TREE ANALYSIS")
print("=" * 80)
print()
print(f"Number of trees at n=7: {len(trees_7)} (expected: 48)")
print()

# Analyze each n=7 tree
tree_data = []

for i, tree in enumerate(trees_7, 1):
    ...
    # Find parent
    parent, operation = find_parent(tree, trees_6)
    ...
    if len(children) == 2:
        ...
        if sorted(child_sizes) == [2, 4]:
            tree_info['special'].append("n=4 + n=2 combination")
        if sorted(child_sizes) == [3, 3]:
            tree_info['special'].append("n=3 + n=3 symmetric combination")
    
    elif len(children) == 3:
        if sorted(child_sizes) == [2, 2, 2]:
            tree_info['special'].append("Triple symmetric: 3 × n=2")
        elif sorted(child_sizes) == [2, 2, 1]:
            tree_info['special'].append("Two n=2 with atom")
        elif sorted(child_sizes) == [3, 2, 1]:
            tree_info['special'].append("n=3 + n=2 + atom combination")
    
    tree_data.append(tree_info)

# Print detailed analysis
for info in tree_data:
    print(f"Tree {info['index']:2d}: {info['tree']}")
    print(f"  Partition: {info['child_sizes']} (children sizes)")
    print(f"  Children: {info['children']}")
    print(f"  Depth: {info['depth']}, Width: {info['width']}")
    
    if info['parent']:
        parent_idx = trees_6.index(info['parent']) + 1 if info['parent'] in trees_6 else '?'
        print(f"  Parent: n=6 Tree {parent_idx} `{info['parent']}`")
        print(f"  Operation: {info['operation']}")
    else:
        print(f"  Parent: SPECIAL COMBINATION (no direct n=6 parent)")
...
print(f"Trees with n=6 parents: {len(with_parents)}")
...
for info in special_combinations:
    print(f"Tree {info['index']}: {info['tree']}")
...
for parent in trees_6:
    children = parent_children.get(parent, [])
    parent_idx = trees_6.index(parent) + 1
    print(f"n=6 Tree {parent_idx:2d}: {parent}")
    if children:
        for child_idx, child_tree, op in children:
            print(f"  → n=7 Tree {child_idx:2d}: {child_tree} ({op})")
    else:
        print(f"  → No direct descendants")
    print()
