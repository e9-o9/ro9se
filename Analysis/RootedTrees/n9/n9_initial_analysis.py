#!/usr/bin/env python3
"""
Analyze n=9 trees to identify:
1. Which trees descend from n=8 parents
2. Which trees are special combinations
3. The partition structure of each tree
4. The complete compositional taxonomy

Expected: 286 trees at n=9 (A000081 sequence: 1, 1, 2, 4, 9, 20, 48, 115, 286, ...)
"""
...
# Generate trees for n=2 through n=9
...
print("=" * 80)
print("N=9 TREE ANALYSIS")
print("=" * 80)
print()
print(f"Number of trees at n=9: {len(trees_9)} (expected: 286)")
print()

# Analyze each n=9 tree
tree_data = []

for i, tree in enumerate(trees_9, 1):
    ...
    # Find parent
    parent, operation = find_parent(tree, trees_8)
    ...
    # Identify special combinations based on partition of n-1 = 8
    ...
    if len(children) == 2:
        ...
        if sorted_sizes == [6, 2]:
            tree_info['special'].append("n=6 + n=2 combination")
        if sorted_sizes == [5, 3]:
            tree_info['special'].append("n=5 + n=3 combination")
        if sorted_sizes == [4, 4]:
            tree_info['special'].append("n=4 + n=4 symmetric combination")
    
    elif len(children) == 3:
        if sorted_sizes == [4, 2, 2]:
        ...
        if sorted_sizes == [2, 2, 2, 2]:
            tree_info['special'].append("Quaternary symmetric: 4 × n=2")
        elif sorted_sizes == [3, 2, 2, 1]:
            tree_info['special'].append("n=3 + 2×n=2 + atom (derived from ternary)")
...
for info in tree_data:
    ...
    if info['parent']:
        parent_idx = trees_8.index(info['parent']) + 1 if info['parent'] in trees_8 else '?'
        print(f"  Parent: n=8 Tree {parent_idx} `{info['parent']}`")
        print(f"  Operation: {info['operation']}")
    else:
        print(f"  Parent: SPECIAL COMBINATION (no direct n=8 parent)")
...
print(f"Trees with n=8 parents: {len(with_parents)}")
...
# For n=9, special combinations come from partitions of 8 into parts >= 2
...
child_counts = {}
for parent in trees_8:
    children = parent_children.get(parent, [])
    parent_idx = trees_8.index(parent) + 1
    child_counts[parent_idx] = len(children)
    if len(children) > 0:
        print(f"n=8 Tree {parent_idx:3d}: {parent}")
...
print(f"Total n=9 trees: {sum(child_counts.values()) + len(special_combinations)}")
...
print("VERIFICATION")
print("=" * 80)
print(f"Generated trees: {len(trees_9)}")
print(f"Expected (A000081): 286")
print(f"Match: {'✓' if len(trees_9) == 286 else '✗'}")
...
print("=" * 80)
print()
print(f"Total Trees: {len(trees_9)}")
